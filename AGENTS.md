...
- 본편은 4층 복도(399,692)에서 시작해 계단 열쇠로 3→2→1층 하강, 1층 수위실에서 front_gate_key를 얻어 현관 탈출.
- 계단 열쇠(현행 배치): 4층 과학 실험실, 3층 방송실, 2층 체력단련실, 1층 행정실. 새 기획서 기준 재배치는 #140에서 진행.
- 미구현(진행 예정): 잉크통 사용 효과. 옥상 씬은 계획 없음. (#139·#140·#141은 완료)
- ⚠️ 이 문서의 나머지 내용은 #159 손도면 개편 이전 상태로 낡았다(맵 크기·계단 좌표·열쇠 배치). 현행은 `CLAUDE.md`를 볼 것.
