...
# ── 세로 밴드 ────────────────────────────────────────────────
NORTH_Y0, NORTH_Y1 = 120, 620        # 북쪽 교실동
MID_Y0, MID_Y1 = 800, 1060           # 중간 띠(화장실·막힌공간)
STAIR_A = (300, 800, 740, 1080)      # 좌측 계단실 (전 층 동일)
BRIDGE_X0, BRIDGE_X1 = 1670, 1890    # 중앙다리(유일한 남북 통로)
BRIDGE_Y0, BRIDGE_Y1 = 1060, 1440    # 위=중간 띠 아래, 아래=남쪽 복도
VOID_Y1 = 1440                       # 공백 구역(건물 밖) 하단 = 남쪽 복도 시작
SOUTH_Y0, SOUTH_Y1 = 1560, 1900      # 남쪽 특별실동(위 복도 1440~1560)
BOT_Y0, BOT_Y1 = 2060, 2360          # 하단 띠
STAIR_B = (1450, 2060, 1890, 2340)   # 중앙 하단 계단실 (2~5층 동일)

# 북쪽 교실 8칸: x0 = 120 + i*398, 폭 368
def north_x(i):
    x0 = 120 + i * 398
    return x0, x0 + 368
...
LAYOUT = {
    4: {
        ...
        "south_left": [("Dasan7", "다산7실", 120, 720), ("CreativeDept", "창의체험부", 750, 1640)],
        "south_right": [("ComputerRoom", "컴퓨터실", 1920, 2400),
                        ("Storage2", "창고", 2430, 2790),
                        ("InfoDept", "정보부실", 2820, 3274)],
        "bottom_left": [("Dasan6", "다산6실", 120, 1200)],
    },
    3: {
        "north": ["1반", "2반", "3반", "생활지도부", "2학년부", "4반", "5반", "6반"],
        "south_left": [("Class6", "6반", 120, 720), ("CareerDept", "진로진학부", 750, 1640)],
        "south_right": [("ScienceLab1", "과학실1", 1920, 2400),
                        ("Storage2", "창고", 2430, 2790),
                        ("ScienceLab2", "과학실2", 2820, 3274)],
        "bottom_left": [("CareerRoom", "진로실", 120, 1200)],
    },
    2: {
        "north": ["1반", "2반", "3반", "1학년부", "4반", "5반", "6반", "7반"],
        "south_left": [("PEStorage", "체육창고", 120, 720), ("PEDept", "체육건강부", 750, 1640)],
        "south_right": [("EduRoom", "교육실", 1920, 2400),
                        ("Storage2", "창고", 2430, 2790),
                        ("ComputerRoom", "컴퓨터실", 2820, 3274)],
        "bottom_left": [],
    },
    5: {   # 프롤로그 전용 — 같은 뼈대, 방 이름만 미술 계열
        "north": ["1반", "2반", "3반", "예술부", "4반", "5반", "6반", "7반"],
        "south_left": [("ArtRoom", "미술실", 120, 720), ("MusicPrep", "음악준비실", 750, 1640)],
        "south_right": [("AVRoom", "시청각실", 1920, 2400),
                        ("Storage2", "창고", 2430, 2790),
                        ("EmptyClass", "빈 교실", 2820, 3274)],
        "bottom_left": [("ArtStorage", "미술창고", 120, 1200)],
    },
}
...
FLOOR1 = {
    "rooms": [   # (키, 라벨, x0, y0, x1, y1, 문 위치)
        ("Class1", "교실1", 120, 1020, 460, 1500, "bottom"),
        ("Class2", "교실2", 490, 1020, 810, 1500, "bottom"),
        ("Class3", "교실3", 840, 1020, 1360, 1500, "bottom"),
        ("YardExit", "운동장 출입구", 1390, 1020, 1920, 1500, None),   # 닫힘
        ("MensRoom1", "남자 화장실", 900, 1950, 1160, 2210, "top"),
        ("WomensRoom1", "여자 화장실", 1190, 1950, 1450, 2210, "top"),
        ("Entrance", "현관", 1600, 1950, 2000, 2280, "top"),           # 탈출구
        ("JanitorRoom", "수위실", 2100, 1950, 2560, 2280, "top"),
        ("Storage1", "창고", 2620, 1950, 2960, 2210, "top"),
        ("Storage2", "창고", 3000, 1950, 3274, 2210, "top"),
    ],
    "staff": (1950, 1020, 3380, 1500),   # 교무실(사선)
    "stair": (220, 1950, 660, 2230),     # 1층 계단 — 다른 층과 위치가 다르다
}
