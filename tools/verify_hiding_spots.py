...
# 격자는 가장 큰 층(3400x2500)을 덮는 상한이다. 1층은 3400x1500이라(#498)
# 아래쪽 행이 남지만, 맵 아래 외벽에 막혀 안에서 새어 나갈 수 없으므로 무해하다.
# **시드는 반드시 맵 안이어야 한다** — 밖에 두면 건물과 이어지지 않은 빈 칸에서
# 퍼져 은신처가 전부 "닿을 수 없다"로 잡힌다.
GRID_WIDTH = 136
GRID_HEIGHT = 100
...
ARRIVE_BY_FLOOR = {
    # 1층은 계단이 한 곳뿐이고, 현관 로비 층으로 바뀌면서(#498) 계단이
    # 하단 띠(800~1480) 왼쪽으로 올라왔다 — `STAIRS[1]`(220, 800, 440x320)의
    # 위쪽 28px 앞, 가운데에서 좌우로 59px 비켜선 자리다.
    1: [(499.0, 772.0), (381.0, 772.0)],
}
