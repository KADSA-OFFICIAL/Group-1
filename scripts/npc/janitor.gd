...
# "260부터는 직선 도주로 못 벗어난다(=#115의 260 되돌림과 같은 상황)"였다.
# #202에서 그 선을 의도적으로 넘어 플레이어와 동일한 320으로 맞춘다 — 회피 수단을
# 속도가 아니라 시야 차단(모퉁이)과 은신(#6)으로 옮기는 시험이다. 직선으로 달리는
# 것만으로는 간격이 벌어지지 않으므로, 되돌리려면 이 값만 250 이하로 낮추면 된다.
