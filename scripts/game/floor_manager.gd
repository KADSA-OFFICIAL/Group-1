...
const FLOOR_BOUNDS := {
	...
	1: Rect2(0, 0, 2280, 1500),   # 현관 로비 층(#498, #554) — 가로 2280px로 콤팩트 축소
...
## 운동장 환경(#356, #520). 밖은 시야가 거의 전체적으로 보이도록 밝기와 마스크를 크게 넓힌다.
const YARD_DARKNESS := Color(0.65, 0.67, 0.74, 1)
## 시야 마스크 배율. 2 = 반경 1024px(389px에서 완전히 검다). 운동장은 25.0으로 넓혀 시야를 가리지 않는다.
const INDOOR_FADE_SCALE := 2.0
const YARD_FADE_SCALE := 25.0
