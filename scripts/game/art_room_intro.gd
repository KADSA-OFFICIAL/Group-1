...
## 3막 — 수위가 돌아오기까지.
##
## **읽는 시간이 걷는 시간보다 크다**(#506). 자막은 글자당 0.05초
## **를 배속으로 나눈 시간**(#563 — 평소 1.5배, 뒤에 줄이 밀려 있으면 2배)
## (`TYPING_SECONDS_PER_CHAR`)로 찍히고 다 찍힌 뒤 3초(`notice_seconds`)를 준다.
## 도입부 단서 넷의 대사를 다 읽는 데만 **24.0초**가 든다:
##
## | 단서 | 대사 | 찍기 + 읽기 |
## |---|---|---|
## | 시우 그림 | 67자 | 6.35초 |
...
## | 국어책 | 29자 | 4.45초 |
##
## 걷는 시간은 격자 BFS 실측이다(`verify_floor_reach`의 격자, 전속력 320px/s):
## 캐비넷 → 창문 1800px(5.6초), → 국어책 → 창문 2400px(7.5초),
## → 남은 단서 2개 → 국어책 → 창문 4800px(15.0초), 단서 넷 전부 순회 3640px(11.4초).
##
## **가장 나쁜 경우는 국어책을 집어 수위를 부른 런이다**(백스톱 방아쇠, #409) —
## 벽 단서 셋을 하나도 못 본 채 3막에 들어간다. 예전 값은 20초였고, 그래서
## **단서를 다 확인하는 것이 산술적으로 불가능했다**(사용자 보고).
##
## 그 런을 런타임으로 재 봤다(자막이 다 뜨는 것을 기다리며 셋을 차례로 조사):
##
## | 구간 | 실측 |
## |---|---|
## | 3막 첫 대사 + 시우 그림 | 9.7초 |
...
## | 날짜 벽 | 6.9초 |
## | 읽기 합계 | **23.0초** |
## | 걷기(캐비넷 → 셋 → 창문, 격자 BFS) | **10.6초** |
## | 필요한 시간 | **33.6초** |
##
## **#563 이후로는 더 여유롭다.** 타이핑이 1.5~2배 빨라져 위 읽기 23.0초가
## 대략 15초로 줄고, 필요한 시간이 33.6 → 26초 안팎이 된다. 44초를 내리지 않은
## 이유: 이 유예는 **되돌릴 수 없는 실패**를 가르는 값이라 여유가 남는 쪽이 낫고,
## 줄이면 그만큼 다시 재야 한다. 아래 계산은 #563 이전 기준이라 상한으로 읽는다.
##
## 44초는 거기에 30% 여유를 둔 값이다. 36초로 잡아 봤더니 여유가 2.5초뿐이었다 —
## 실제 플레이어는 헤매고, 8초 남았을 때 뜨는 경고(`WARN_AT`)가 걷는 도중에 온다.
## 곧바로 창문으로 가는 런(걷기 5.6초)에는 넉넉하지만, 그 런은 이 시간을 쓸 일이 없다.
##
## **책 유무로 가르지 않는다.** #477에서 걷는 거리(1800 vs 2400px = 1.9초)로
## 갈랐는데 30초 예산에서 1.9초는 6%이고, 읽는 시간은 8초 이상 차이가 나며
## 그 방향이 **반대**다 — 책을 든 런(= 책이 방아쇠였던 런)이 안 읽은 벽 단서가
## 셋이라 더 오래 걸린다. 44초가 두 경우를 다 덮는다.
## #477의 안내(1막·3막 국어책 대사·화면 표시·창문 거절 문구)는 그대로 남는다.
const GRACE_SECONDS := 44.0
const WARN_AT := 8.0
...
func _act3_grace(gs) -> void:
	...
	# 유예는 책 유무로 가르지 않는다 (#506) — 근거는 `GRACE_SECONDS` 주석에 있다.
