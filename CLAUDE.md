...
- 본편은 4층 복도(399,692)에서 시작해 계단 열쇠로 3→2→1층 하강, 1층 수위실 금고에서 front_gate_key를 얻어 현관 탈출.
- 계단 열쇠(#140 재배치, 각 열쇠 2곳): stair_key_4 = 4층 다산실·과학 실험실 / stair_key_3 = 4층 수학교구실(태호 쪽지)·3층 교무실 여벌 고리 / stair_key_2 = 3층 교무실 열쇠 보관함·2층 샤워실 배수구 / stair_key_1 = 1층 행정실(되돌아 올라갈 때만 필요).
- 단서 오브젝트(#140): 4층 다산실(하람 메모, found_songharam)·상담실(시우 상담기록 + 시우 그림 재조사)·인쇄실(대응 매뉴얼·잉크통)·과학 실험실(found_jominhyuk)·역사자료실(read_siwoo_past), 3층 방송실(found_imnayeon)·학생회실(전단)·교무실(종태 주의 기록), 2층 체육관 입구(found_kangyujin)·샤워실(끌린 자국)·교무실(found_baekseungho), 1층 교장실(read_principal_letter)·수위실(사진벽·학생증·공책 read_janitor_notebook·금고).
- 엔딩 판정은 exit_door.gd 상수(HIDDEN_ENDING_FLAGS / REPORT_CHOICE_FLAGS) → EndingScene.pending_ending. 숨은 엔딩은 1층에서 공책을 읽은 뒤 stair_key_1로 4층 상담실 그림을 재조사하고 다시 내려와야 열린다.
