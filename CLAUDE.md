...
- 층 구조: 북쪽 교실 열(2·3·4층 9칸, 5층 8칸, **`NORTH_GAP=0`이라 반끼리 벽을 맞댄다** — 예전 32px 간격은 복도로 열린 막다른 골목이었다) → 북쪽 복도 → 중간 띠(좌: 계단실·남녀화장실·막힌공간 / 우: 창고·막힌공간·남녀화장실, **사선**) → 공백 구역을 가로지르는 **중앙다리**(유일한 남북 통로) → 남쪽 복도·특별실동 → 하단 띠(대형실·계단실·화장실). 1층만 도면이 달라 아래쪽 절반만 건물(교실1~3·운동장출입구·교무실 / 현관·수위실·창고).
...
- 방·복도 집기: 방이 빈 사각형이라 들어갈 이유가 없던 것을 `gen_floors.py`의 `add_props()`가 채운다. 노드는 충돌 `PC_`(StaticBody2D `PropBodies`) + 시각 `PV_`(`Props`) 한 쌍, 충돌 없는 장식은 `PD_`(칠판·게시판)다. **벽과 달리 광원 차단체를 달지 않는다**(낮은 집기 설정 + 조명 튜닝 #74를 흔들지 않으려는 목적)—그래서 `verify_scenes`의 벽↔차단체 1:1 검사에 걸리지 않는다.
  - 교실은 전용 배치 `_classroom()`: 앞벽 칠판(장식)→교탁→책상+의자 격자→뒷벽 청소도구함·사물함·쓰레기통, 옆벽에 창문(왼쪽)·게시판(오른쪽) 장식. **중앙 통로를 문과 같은 x에 내므로 방 중심이 항상 빈다** — 중심을 막으면 `verify_floor_reach`가 방을 도달 불가로 본다. 뒷벽 사물함은 복도용보다 좁은 `CLASS_LOCKER_W`/`CLASS_BACK_PAD`를 쓴다(복도 규격이면 북쪽 교실에서 한 칸도 안 들어간다).
  - 그 밖의 방은 `prop_kind()`로 종류를 정하고 `PROP_SPECS`의 단위를 깐다. 단위 크기는 최대치라 좁은 자리에서는 `PROP_MIN_W/H`까지 줄어든다(규격을 고집하면 화장실·현관이 통째로 빈다).
  - 복도는 `add_corridor_props()`가 맞닿은 방 벽마다 사물함을 붙이고 남는 벽면에 소화기·게시판을 건다. 바닥 유도선은 `add_corridor_lines()`가 **먼저** 낸다(나중 노드가 위에 그려진다). 복도 띠는 빌더가 넘긴다(2~5층 3개, 1층 1개). 문 틈 양옆 `CORR_DOOR_PAD`는 비운다 — 수위 순찰의 문 앞 대기 지점이 거기다. 벽면이 복도 위/아래 어느 쪽인지에 따라 장식 앵커가 뒤집히므로 `face()`로 두께마다 맞춘다.
...
- 계단 열쇠(#159 재배치): stair_key_4 = 4층 다산7실·컴퓨터실 / stair_key_3 = 4층 창고(태호 쪽지)·3층 2학년부 여벌 고리 / stair_key_2 = 3층 2학년부 보관함·2층 하단 남자화장실 배수구 / stair_key_1 = 1층 창고(하강엔 불필요).
