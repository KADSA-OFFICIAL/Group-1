...
- **벽 앞면(#268)**: `sc.wall()`이 **가로 벽에만** 아래쪽 앞면(`WF_`)을 하나 더 낸다. 위에서 내려다본 16px 띠 하나만 그리면 두께만 있고 높이가 없어 건축 도면처럼 보인다 — 면 하나가 벽을 세운다. 세로 벽에는 그리지 않는다(탑다운에서 앞면이 보이는 건 시선과 마주 보는 가로 벽뿐). **시각 전용이라 충돌·광원 차단체는 그대로 16px 띠에 있다.**
...
- 바닥 레이어: **`Floor`(맵 바탕) → `Ground`(복도 마루·널 이음매) → `Rooms`(방 바닥) → `RoomMarks`(방 바닥 표시) → `Props`(집기) → `Stairwells` → `WallGlow`(벽·문·라벨, CanvasLayer)** 순으로 그려진다. **바닥에 그리는 것(`FM_`, `sc.floor_mark`)과 벽·가구에 붙는 것(`PD_`, `sc.decor`)은 레이어가 다르다** — 화장실 바닥 타일을 장식으로 냈더니 가구 밑에 깔리는 게 정상인데 `verify_props`가 집기와 겹쳤다고 잡았다. 벽면에 붙는 것은 `WD_`(`sc.wall_decor`)로 WallGlow 안에 낸다(걸레받이·기둥). **먼저 선언된 형제가 먼저 그려지므로 선언 순서가 곧 레이어다.** 방 바닥은 `room_floor()`가 종류별로 칠하고(교실 마루·화장실 타일·창고 시멘트…), 막힌 공간은 더 어둡다. 복도는 `add_ground()`가 마루 + 널 이음매를 낸다. **널은 복도를 가로지르는 세로 방향**이고, 선은 폴리곤이 아니라 `floor_board` **무늬**가 낸다(#268). 복도와 같은 방향으로 눕혔더니 선이 끝에서 끝까지 이어져 마루가 아니라 줄 친 종이로 보였고, 직각으로 돌려 폴리곤으로 그렸더니 층당 150개가 넘어 노드 예산을 넘겼다. 무늬로 옮기니 노드 0개에 타일 격자(32px)와도 정확히 맞는다 — 그래서 `floor_board`는 이제 `LINELESS`가 아니다. 화장실 줄눈은 아직 폴리곤(`FM_`, 60px)이라 `floor_matte`는 계속 선이 없어야 한다.
  창문은 짙은 밤하늘색 유리이고 **창 아래 방 바닥에 달빛 자국**(`FM_`)이 깔린다. 광원이 아니라 바닥 표시라 조명 튜닝(#74)에 영향이 없다. `add_ceiling_lights()`의 꺼진 형광등까지 전부 충돌 없는 시각 전용이다.
