...
- 바닥 레이어: **`Floor`(맵 바탕) → `Ground`(복도 바닥·타일 이음매) → `Rooms`(방 바닥) → `Props`(집기) → `Stairwells` → `WallGlow`(벽·문·라벨, CanvasLayer)** 순으로 그려진다. **먼저 선언된 형제가 먼저 그려지므로 선언 순서가 곧 레이어다.** 방 바닥은 `room_floor()`가 종류별로 칠하고(교실 마루·화장실 타일·창고 시멘트…), 막힌 공간은 더 어둡다. 복도는 `add_ground()`가 리놀륨색 + 타일 이음매를, `add_ceiling_lights()`가 꺼진 형광등을 낸다 — 전부 충돌 없는 시각 전용이다.
- 방·복도 집기: 방이 빈 사각형이라 들어갈 이유가 없던 것을 `gen_floors.py`의 `add_props()`가 채운다. 노드는 세 갈래다 — 충돌 `PC_`(StaticBody2D `PropBodies`) + 시각 `PV_`(`Props`) 한 쌍, 충돌 없이 벽에 붙는 장식 `PD_`(칠판·게시판·창문·시계·소화기), 집기 위에 놓이는 소품 `PT_`(책상 위 교과서·선반의 책). `PD_`는 집기와 겹치면 안 되고 `PT_`는 반대로 어느 집기 안에 온전히 들어가야 한다 — `verify_props`가 각각 확인한다. **벽과 달리 광원 차단체를 달지 않는다**(낮은 집기 설정 + 조명 튜닝 #74를 흔들지 않으려는 목적)—그래서 `verify_scenes`의 벽↔차단체 1:1 검사에 걸리지 않는다.
  ...
  - **교실 문은 미닫이문**이다(`add_sliding_doors()` + `scripts/interactions/sliding_door.gd`). 문 틈에 짝 두 개를 세우고, 다가오면 양옆 벽 속으로 밀린다(E 불필요 — 교실 문이 층마다 8~10개라 조사형이면 이동이 조작으로 뒤덮인다). 패널 폴리곤은 **절대 좌표**로 낸다 — 검사 스크립트들이 노드 position을 반영하지 않고 폴리곤을 그대로 읽는다.
...
- 도트 타일 텍스처(#243): 바닥·벽·집기 `Polygon2D`는 단색이 아니라 **`tools/gen_tiles.py`가 구운 회색조 도트 무늬**(`assets/tiles/*.png`)를 물고 있다(표준 라이브러리만, 결정론적). 무늬만 텍스처이고 **색은 `gen_floors.py`의 팔레트가 계속 쥔다** — 방 종류별 바닥색(#237)이 그대로 살아 있게. 색 → 타일 대응은 `gen_floors.py`의 `TEX` 표이고, 여기 없는 색(열쇠·화살표·복도 유도선·천장등)은 단색으로 남는다. 무늬를 고치려면 `gen_tiles.py`의 무늬 함수를 고치고 다시 돌린 뒤 `gen_floors.py`도 재생성한다. 규약 셋: ① 무늬 최소 단위는 월드 2px(`DOT`) — 카메라 zoom이 1.25라 1px 무늬는 화면에서 들쭉날쭉해진다. ② 모든 타일의 평균 휘도는 `MEAN`(0.78)로 고정하고 `gen_floors`가 텍스처를 붙인 색에만 `1/TEX_MEAN` 게인을 준다 — 그래서 평균 밝기가 텍스처 도입 전과 같고 조명 튜닝(#74)이 흔들리지 않는다(두 상수는 같이 고친다). ③ UV를 비워 정점 좌표를 UV로 쓰므로 맵 전체가 하나의 타일 격자에 정렬된다. `texture_filter = 1`(Nearest)·`texture_repeat = 2`는 층 씬 루트와 `WallGlow/RoomWallVisuals`에만 걸고 자식이 물려받는다 — **CanvasLayer 직속 노드(계단 난간)만 예외로 직접 적는다**(물려받을 부모가 없다).
...
- 타일 무늬를 고쳤으면 `python3 tools/gen_tiles.py` 후 `python3 tools/gen_floors.py`까지
  다시 돌립니다(생성기가 평균 휘도·명암·주기성·도트 격자를 스스로 검사하고,
